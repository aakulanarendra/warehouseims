...
// Services
import { getNewProducts, getBestsellersProducts, getFeaturedProducts } from "../../services";

// Multilanguage
import { withTranslate } from 'react-redux-multilingual'

export default class BlockProducts extends React.Component {
	componentDidMount() {
 	...
 	render() {
		...
		return (
			<div className="box clearfix box-with-products with-scroll box-no-advanced">
				<div className="box-heading">{this.props.title}</div>
					...
					{this.props.carousel === 'true' && products.length > 0 ? (
						...
						<div className="product-grid">
							<div className="row">
								{products.map(product => {
									return (
				        				<div className={productsClass} key={product.id}>
											<ProductCard
												key={product.id}
												{...product}
											/>
